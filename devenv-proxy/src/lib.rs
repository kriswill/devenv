...
use pingora_core::{listeners::ConnectionFilter, server::Server, upstreams::peer::HttpPeer};
...
impl ProxyHttp for Router {
    ...
    async fn request_filter(
        ...
        *ctx = host.as_deref().and_then(|host| self.routes.resolve(host));
    ...
    async fn upstream_peer(
        ...
        let upstream = ctx.expect("routed requests always have an upstream");
...
pub fn run(listen: SocketAddr, control_socket: &Path) -> Result<()> {
    ...
    let mut server = Server::new(None).context("failed to initialize Pingora")?;
    server.bootstrap();
